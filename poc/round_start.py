...
async def round_start():
    ...
    print("{} candidates, {} slots".format(len(common.POC_HYPER_NODES_LIST), common.MAX_ROUND_SLOTS))
    tickets = await determine.hn_list_to_tickets(common.POC_HYPER_NODES_LIST)
    print("Tickets\n", tickets)
    slots = await determine.tickets_to_delegates(tickets, common.POC_LAST_BROADHASH)
    print("Slots\n", slots)
    test_slots = await determine.hn_list_to_test_slots(common.POC_HYPER_NODES_LIST, slots)
    print("Tests Slots\n", test_slots)
    print("")
