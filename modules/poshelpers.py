        ...
        for chunk in r.iter_content(chunk_size=1024):
            if chunk:
                ...
                if chunkno % 10000 == 0:  # every x chunks
                    app_log.info(
                        "Downloaded {} %".format(int(100 * (chunkno / total_size)))
                    )
...
def update_source(url: str, app_log: logging.log = None):
    ...
    try:
        ...
        if app_log:
            app_log.warning(
                "Something went wrong in update_source: {}, aborted".format(e)
            )
        raise
...
def first_height_is_better(height_a: dict, height_b: dict) -> bool:
    """
