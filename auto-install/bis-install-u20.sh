#!/bin/bash
# 2019 - 2021 Bismuth Foundation and NodeSupply
# Distributed under the MIT software license, see http://www.opensource.org/licenses/mit-license.php.

# Usage: bash ./bis-install.sh
# or one liner : curl https://raw.githubusercontent.com/bismuthfoundation/hypernode/beta99/auto-install/bis-install-u20.sh|bash
# Setup a regular Bismuth node and hypernode on a fresh Ubuntu 20.04 install.
...
download_node() {
    ...
    # use default python3, 3.8 on ubuntu 20
    sed -i -e "s/python3.7'/python3'/g" ./node_sentinel.py
    echo "Installing PIP requirements"
    python3 -m pip install setuptools ipwhois
...
add_cron_jobs() {
	...
	if ! crontab -l | grep "cron5.py"; then
	  (crontab -l ; echo "*/10 * * * * cd /root/hypernode/crontab;python3 cron5.py") | crontab -
	fi
}
