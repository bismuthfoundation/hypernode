...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="count",
        default=False,
        help="Read until last block, do not use real limit.",
    )
    parser.add_argument("-r", "--round", default=0, help="Query for up to that round")
    parser.add_argument(
        "-I", "--ip", type=str, default="", help="Filter events for that IP only."
    )
    parser.add_argument(
        "-P",
        "--pow",
        type=str,
        default="",
        help="Filter events for that PoW address only.",
    )
    parser.add_argument(
        "-b",
        "--balancecheck",
        action="count",
        default=False,
        help="Do an extra global balance check at the end.",
    )
