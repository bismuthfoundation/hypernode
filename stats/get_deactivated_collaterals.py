...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Bismuth HyperNode Collateral test")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="count",
        default=False,
        help="Use the distinct process (debug)",
    )
    ...
    poschain = poschain.SqlitePosChain(
        verbose=verbose, app_log=app_log, db_path=datadir, mempool=None
    )
    ...
    res = loop.run_until_complete(
        pow.load_hn_pow(
            datadir="../main/data",
            a_round=int(round + 1),
            inactive_last_round=inactive_hns,
            force_all=False,
            no_cache=True,
            ignore_config=True,
    ...
    print(collateral_dropped)
    for detail in collateral_dropped:
        openfield = "{}:{}:{},reason=Collateral drop".format(
            detail["ip"], detail["port"], detail["pos"]
        )
        bis_url = pow_interface.create_bis_url(
            recipient=detail["pow"],
            amount="0",
